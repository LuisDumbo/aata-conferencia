<section class="relative">
    <div class="max-w-6xl mx-auto px-4 py-10">
        ...
        <div class="flex items-end justify-between gap-4 mb-6">
            <div>
                <h2 class="text-3xl font-extrabold text-slate-900">{{ t('salas.gallery_title') }}</h2>
                <p class="text-slate-600">{{ t('salas.gallery_subtitle') }}</p>
            </div>
        </div>
        ...
        @if ($hasMore)
            <div class="mt-10 flex justify-center">
                <button wire:click="loadMore" wire:loading.attr="disabled"
                    ...
                    <span wire:loading.remove>{{ t('salas.load_more') }}</span>
                    <span wire:loading>{{ t('salas.loading') }}</span>
                </button>
            </div>
        @endif
    ...
    <div x-data="{ show: false }" x-init="window.addEventListener('scroll', () => { show = window.scrollY > 400 })" class="fixed bottom-6 right-6 z-50">
        <button x-show="show" x-transition @click="window.scrollTo({ top: 0, behavior: 'smooth' })"
            ...
            :aria-label="'{{ t('salas.scroll_top') }}'" :title="'{{ t('salas.scroll_top') }}'">
            ↑
        </button>
    </div>
</section>
