<section class="relative">
    <div class="max-w-6xl mx-auto px-4 py-10">
        ...
        <div class="flex items-end justify-between gap-4 mb-6">
            <div>
                <h2 class="text-3xl font-extrabold text-slate-900">{{ t('palestrantes.gallery_title') }}</h2>
                <p class="text-slate-600">{{ t('palestrantes.gallery_subtitle') }}</p>
            </div>
        </div>
            ...
            @foreach ($speakers as $sp)
                <a href="{{ route('speakers.show', $sp['slug']) }}"
                    ...
                    <div class="absolute bottom-0 left-0 right-0 p-5 text-center">
                        <h3 class="text-white font-extrabold text-lg leading-tight">{{ $sp['name'] }}</h3>
                        <p class="mt-2 text-xs uppercase tracking-wide text-indigo-100/90">{{ $sp['role'] }}</p>
        ...
        @if ($hasMore)
            <div class="mt-10 flex justify-center">
                <button wire:click="loadMore" wire:loading.attr="disabled"
                    ...
                    <span wire:loading.remove>{{ t('palestrantes.load_more') }}</span>
                    <span wire:loading>{{ t('palestrantes.loading') }}</span>
                </button>
            </div>
        @endif
    ...
    <div x-data="{ show: false }" x-init="window.addEventListener('scroll', () => { show = window.scrollY > 400 })" class="fixed bottom-6 right-6 z-50">
        <button x-show="show" x-transition @click="window.scrollTo({ top: 0, behavior: 'smooth' })"
            ...
            :aria-label="'{{ t('palestrantes.scroll_top') }}'" :title="'{{ t('palestrantes.scroll_top') }}'">
            ↑
        </button>
    </div>
</section>
